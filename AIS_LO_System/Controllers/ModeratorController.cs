...
namespace AIS_LO_System.Controllers
{
    ...
    public class ModeratorController : Controller
    {
        ...
        public async Task<IActionResult> Inbox()
        {
            ...
            // Load document data for each pending submission so we can render inline
            var outlineUrls = new Dictionary<int, string?>();       // submissionId → url
            var assignments = new Dictionary<int, Assignment?>();    // submissionId → assignment
            var rubrics = new Dictionary<int, Rubric?>();           // submissionId → rubric
            var markData = new Dictionary<int, (Assignment? asgn, List<StudentAssessmentMark> marks, Rubric? rubric, List<StudentCriterionMark> criterionMarks)>();
            ...
            foreach (var sub in pending)
            {
                switch (sub.ItemType)
                {
                    case SubmissionItemType.CourseOutline:
                        var baseName = $"{sub.CourseCode}-{sub.Year}-T{sub.Trimester}";
                        ...
                        foreach (var ext in new[] { ".pdf", ".docx" })
                        {
                            ...
                            {
                                url = $"/uploads/outlines/{baseName}{ext}";
                                break;
                            }
                        }
                        outlineUrls[sub.Id] = url;
                        break;

                    case SubmissionItemType.AssignmentDocument:
                        var asgn = await _context.Assignments
                            .Include(a => a.Files)
                            .FirstOrDefaultAsync(a => a.Id == sub.ItemRefId);
                        assignments[sub.Id] = asgn;
                        break;
                    ...
                    case SubmissionItemType.Rubric:
                        var rubric = await _context.Rubrics
                            ...
                            .FirstOrDefaultAsync(r => r.Id == sub.ItemRefId);
                        if (rubric != null)
                            foreach (var c in rubric.Criteria)
                                c.Levels = c.Levels.OrderByDescending(l => l.Score).ToList();
                        rubrics[sub.Id] = rubric;
                        break;

                    case SubmissionItemType.StudentMarks:
                        var marksAsgn = await _context.Assignments
                            .FirstOrDefaultAsync(a => a.Id == sub.ItemRefId);
                        var marks = marksAsgn != null
                            ? await _context.StudentAssessmentMarks
                                .Include(m => m.Student)
                                .Where(m => m.CourseCode == sub.CourseCode && m.AssessmentName == marksAsgn.AssessmentName)
                                .ToListAsync()
                            : new List<StudentAssessmentMark>();
                        var marksRubric = marksAsgn != null
                            ? await _context.Rubrics
                                .Include(r => r.Criteria).ThenInclude(c => c.Levels)
                                .FirstOrDefaultAsync(r => r.AssignmentId == marksAsgn.Id)
                            : null;
                        var criterionMarks = marksAsgn != null
                            ? await _context.StudentCriterionMarks
                                .Where(x => x.AssignmentId == marksAsgn.Id)
                                .ToListAsync()
                            : new List<StudentCriterionMark>();
                        markData[sub.Id] = (marksAsgn, marks, marksRubric, criterionMarks);
                        break;
                }
            }

            ViewBag.OutlineUrls = outlineUrls;
            ViewBag.Assignments = assignments;
            ViewBag.Rubrics = rubrics;
            ViewBag.MarkData = markData;
